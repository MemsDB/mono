...
exports.QueryHandler = exports.QueryBuilder = exports.runQuery = void 0;
 ...
 * @category Query
 */
const compare = (doc, query, queryPlugins) => {
    const val = Indexed_1.getOrCreateIndex({ doc, query });
    ...
    switch (op) {
        case '<':
            ...
            res = Indexed_1.getOrCreateIndex({ doc, query }) < comp;
            break;
        case '>':
        ...
        case '===':
            ...
            const data = Indexed_1.getOrCreateIndex({ doc, query });
            res = data === comp;
            break;
        case 'includes':
        ...
        default:
            ...
            if (plugin)
                return plugin(query.key, Indexed_1.getOrCreateIndex({ doc, query }), comp);
            return false;
    }
 ...
 * @category Query
 */
const runQuery = (queryArr, col, seedDocs, nested_, nestedOp_) => {
    let queries = [];
    ...
    if (queryArr.constructor.name === 'QueryBuilder') {
        queries = queryArr.queries;
    }
    else
        queries = queryArr;
    /* DEBUG */ _('Querying collection `%s`. %d queries left to execute', col.name, queries.length);
    return queries.reduce((docs, query, i, queries) => {
        // Return filtered documents if there are none left, or if the query array is empty
        if (docs.length === 0) {
            /* DEBUG */ _('Query on collection `%s` completed, %d documents left with %d queries left to execute', col.name, docs.length, queries.length - i);
            return docs;
        }
        ...
        if (query.key === '_updatedAt' ||
            ...
            query.key === 'id' ||
            NestedKey_1.nestedKey(col.schema, query.key) !== undefined ||
            (query.key === '' &&
                (query.operation === '&&' || query.operation === '||') &&
                Array.isArray(query.comparison))) {
            /* DEBUG */ _('Collection contains key `%s`, querying key with operator `%s`', query.key, query.operation);
            ...
            switch (query.operation) {
                case '||':
                    ...
                    query.comparison.map((orQuery) => tmpOr.push(
                ...
                case '&&':
                    ...
                    const tmpAnd = exports.runQuery(query.comparison, col, docs, _, '&&');
        ...
        /* DEBUG */ _('Query on collection `%s` continuing with %d documents left and %d queries left to execute', col.name, docs.length, queries.length - i);
        return docs;
    }, seedDocs);
};
...
class QueryBuilder {
     ...
     */
    where(key, operation, comparison, inverse = false) {
        this.queries.push({
            key,
            operation,
            comparison,
     ...
     * @param inverse Inverse the result of the where query
     */
    static where(key, operation, comparison, inverse = false) {
