...
const nestedKey = (obj, key = '') => {
    ...
    // requested
    if ((key === '_updatedAt' || key === '_createdAt' || key === 'id') &&
        !Array.isArray(obj)) {
        return obj[key];
    }
            ...
            if (Array.isArray(tmpProp) &&
                ...
                tmpVal = tmpProp
                    ...
                    // contents if we have a particularly long key string
                        ...
                        return exports.nestedKey(val, keys.slice(i + 1).join('.'));
                })
