...
const core_1 = require("@memsdb/core");
class MemsDBServer {
    constructor(db, opts = {}) {
        /**
        ...
        this.DBEventHandlers = [
            new core_1.EventHandler('EventDBAddCollection', event => {
                const { collection } = event;
                this.registerCollection(collection);
            }),
        ];
        const { port = 16055, secret = uuid_1.v4(), rootPassword = uuid_1.v4(), tokenExpiry = 86400, requireAuth = 'all', } = opts;
        ...
        const [rootUser] = authCollection.find(core_1.QueryBuilder.where('username', '===', 'root'));
        // If no root user was found, create one
        if (!rootUser) {
            /* DEBUG */ _('root user not found, creating new user');
            /* DEBUG */ _('Generating salt for root user');
            const salt = crypto_1.randomBytes(256).toString('base64');
    ...
    registerCollection(collection) {
        ...
        this.app.put(`/collection/${collection.name}/insert`, (req, res) => {
            ...
            const docObject = smoldash_1.merge(smoldash_1.cloneDeep(collection.schema), docData);
    ...
    memsdbPopulate(_debug, req, collection, docs) {
        ...
        if (populateQuery !== '') {
            ...
            const slice = core_1.populate(collection, docs, populateQuery, filter);
    ...
    parseAuthToken(req) {
        ...
        try {
            const token = jsonwebtoken_1.verify(AuthToken, this.secret, {
    ...
    createAuthToken(req) {
        ...
        let query = core_1.QueryBuilder.where('type', '===', authType);
        if (authType === 'api') {
        ...
        if (authType === 'api' && authAccount.data.username !== 'root') {
            ...
            existingTokens = tokensCol.find(new core_1.QueryBuilder()
        ...
        /* DEBUG */ _('Signing JWT');
        const jwt = jsonwebtoken_1.sign(jwtObj, this.secret, {
    ...
    static saltHashString(str, salt) {
        const hash = crypto_1.pbkdf2Sync(str, salt, MemsDBServer.keyIterations, MemsDBServer.keyLength, 'sha512');
...
exports.MemsDBServer = MemsDBServer;
/**
